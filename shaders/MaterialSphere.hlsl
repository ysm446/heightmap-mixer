...
struct SphereConstants
{
    ...
    uint metallicIndex;
    uint aoIndex;
    uint mapChannels;        // 4bit ずつ MM_CHANNEL_SLOT_* の順
    ...
    // 0 以外なら法線マップの緑を反転して読む。**いまは常に 0。**
    // 規約（OpenGL / DirectX）の切り替えはマテリアルがまだ持たないので、
    // 合成（CompositeLayer）とサムネイル（MaterialThumbnail）と同じく素通しする。
    uint flipNormalGreen;
...
void CsMain(uint3 dispatchThreadId : SV_DispatchThreadID)
{
    ...
    {
        roughness = SampleScalarMap(g_sphere.roughnessIndex, MM_CHANNEL_SLOT_ROUGHNESS, uv,
    ...
    {
        metallic = SampleScalarMap(g_sphere.metallicIndex, MM_CHANNEL_SLOT_METALLIC, uv,
    ...
    {
        ambientOcclusion = SampleScalarMap(g_sphere.aoIndex, MM_CHANNEL_SLOT_AO, uv,
                                           MapLod(g_sphere.aoIndex, deltaX, deltaY));
    }

    // --- 法線 --------------------------------------------------------------
    // 接空間は緯度経度の貼り方から作る。u が増える向きが接線、v が増える向き
    // （北極から南へ）が従法線。**合成と同じく、マップは素通しで読む**
    // （flipNormalGreen は常に 0）。
