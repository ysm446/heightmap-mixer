...
bool PropertyFloat(const char* label, float* value, float minValue, float maxValue,
                   ...
                   float snapStep = 0.0f);
// outActive を渡すと、スライダーを掴んでいる（または Ctrl + クリックで入力中の）間 true が入る。
// 反映すると画面そのものが動く値（文字サイズなど）は、これが false になるまで
// 適用を待つ。ドラッグ中に反映すると、掴んでいるスライダーごと動いて狙えない。
bool PropertyInt(const char* label, int* value, int minValue, int maxValue, int defaultValue,
                 const char* tooltip = nullptr, bool* outActive = nullptr);
