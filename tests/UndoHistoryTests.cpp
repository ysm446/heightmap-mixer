...
using mm::DocumentSnapshot;
using mm::UndoHistory;
using mm::tests::Check;
using mm::tests::Section;

// 段を見分けるため、レイヤーの枚数を目印に使う。
DocumentSnapshot MakeSnapshot(size_t layerCount) {
    DocumentSnapshot snapshot;
    snapshot.layers.resize(layerCount);
    return snapshot;
}

size_t LayerCount(const DocumentSnapshot& snapshot) {
    return snapshot.layers.size();
}
