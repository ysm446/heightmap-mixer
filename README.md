...
| Ctrl + Z / Y | 元に戻す / やり直す（レイヤーとマテリアルが対象） |
...
- **レイヤー**（右・タブ）— レイヤーの追加 / 複製 / 削除と、選択中レイヤーの編集。
  **幅があるときは「一覧 | プロパティ」の 2 列**で、選び直しながら調整できる。
  一覧は上が最前面で、ドラッグして並べ替える。
- **マテリアル**（右・タブ）— PBR のマップ一式に名前を付けたもの。
...
- **プレビュー設定**（右・タブ）— 形状、合成結果の使用可否、UV スケール、変位量、
  テセレーション、合成解像度、カメラ（焦点距離）。
- **ライティングと露出**（右・タブ）— ライト（影のオンオフを含む）、露出、
...
- **表示** — 垂直同期、シェーダのホットリロード、背景色。

設定は `%LOCALAPPDATA%/material-mixer/settings.json` に保存される。
