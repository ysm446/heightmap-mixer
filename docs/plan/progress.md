...
更新日時: 2026-08-31 11:52

## 現在の状況

**M3a（合成エンジン / 単色・ノイズレイヤー）完了。** Debug / Release ともに
ビルドが通り、レイヤースタックをタイル分割して GPU で評価し、
ハイトベースブレンドと RNM で合成した結果を PBR プレビューに貼れる。
既定は「岩の隙間に砂が溜まる」2 レイヤー構成。次は M3b（テクスチャレイヤー）。
...
1. LDR 画像（PNG / TGA / JPG）の読み込みを `core/ImageIo` に追加。
2. テクスチャライブラリ（読み込み済みテクスチャの保持と bindless インデックスの払い出し）。
3. レイヤーのチャンネルごとにテクスチャを指定できるようにする。
4. マスクテクスチャ。
5. レイヤー UI にテクスチャの指定欄を追加。
