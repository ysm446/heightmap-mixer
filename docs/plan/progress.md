...
- 2026-09-04 21:40 — **マテリアルに色相と彩度を追加**。派生アプリ（terrain-graph）と
  同じ仕様に揃えた。ベースカラーだけに、ティントの後に掛かる。計算はリニア空間のまま
  `AdjustBaseColor`（`shaders/CompositeCommon.hlsli`）に集約し、合成・サムネイル・
  マテリアルプレビューの球の 3 か所すべてがこの関数を通る。ファイル形式は
  キーの追加だけなので版は据え置き（版 3 のまま）。

- 2026-09-04 20:30 — **テクスチャ / マテリアル / 天球の一覧をサムネイルだけにし、
  プレビューの窓を 3 つ揃えた**。派生アプリ（terrain-graph）で先に整えた形に合わせた。
  一覧からボタンの帯と設定の行を外し、操作は右クリックのメニューへ、
  中身の確認と設定はサムネイルのダブルクリックで開く窓へ移した。
  窓は 3 つとも上下 2 区画（上がプレビュー、下がプロパティ）で同じ作法。
  マテリアルと天球のプレビューは新しい `renderer::MaterialSphere` /
  `renderer::SkySphere`（どちらもレイと単位球の解析的な交差、コンピュートで
  1024² へ描く）で、照らし方・露出・トーンマップはビューポートに合わせてある。
  規則は [design/design-guide.md](../design/design-guide.md) の
  「プレビューの窓」と「5. 一覧」に書いた。

- 2026-09-02 12:38 — **アプリアイコンを追加**。
  岩・土・草の地層と地表の植生を描いたアイコンを、実行ファイルと Win32 ウィンドウへ
  設定した。16〜256px の複数サイズを `.ico` に収録している。
...
- **文字サイズ（`設定 > UI > 文字サイズ`、11〜28px）も同じく動的ラスタライズ**
  （`style.FontSizeBase`）。`ui::ApplyTheme()` はスタイルを既定から作り直すので、
  呼んだあとは `ImGuiLayer::ApplyScaleToStyle()` で `FontSizeBase` / `FontScaleDpi`
  を入れ直す。文字が入る寸法は `ui::TextScaled()` を通し、余白は `ui::Scaled()` のまま。
