...
M5b（プロジェクトの保存と読み込み）。土台は M5a で入れてある。

1. JSON でレイヤースタック / マテリアル / テクスチャの参照 / 各種設定を保存する。
2. **ペイントマスクは手続きで再現できないので、PNG として一緒に保存する。**
   `PaintMaskStore::ReadPixels` / `AddFromPixels` を使う。
3. アセットのパスはプロジェクトからの相対で持ち、プロジェクトごと移動できるようにする。
4. ファイルメニュー（新規 / 開く / 保存 / 名前を付けて保存）と `--project` 起動オプション。
5. 未保存の変更があるまま閉じたときの確認は、まだ無い。M5b で入れるか判断する。
