...
**M3 完了（M3a: 合成エンジン / M3b: テクスチャレイヤー）。**
レイヤースタックをタイル分割して GPU で評価し、ハイトベースブレンドと RNM で
合成した結果を PBR プレビューに貼れる。レイヤーの各チャンネルに画像を
割り当てられ、ハイトテクスチャによる合成も動作する。次は M4（マスク生成）。
...
1. プロシージャルノイズの拡充（Worley / Voronoi、現在は fBm のみ）。
2. 合成の中間結果に由来するマスク（高度 / 傾斜 / 曲率 / AO）。
   これが Mixer らしさの中核で、「傾斜の急な面にだけ岩を出す」などが書けるようになる。
3. マスクのカーブとレベル調整の拡充。
4. ペイントマスク（ブラシ）。
...
- **コンピュートシェーダでは暗黙の LOD が使えない**: `Sample` ではなく
  `SampleLevel` を使い、出力テクセル 1 つが張る UV 幅からミップレベルを計算する。
  これを怠ると縮小時にひどくエイリアスする。

- **sRGB とリニアの両方の SRV を張る**: 同じ画像をベースカラーにもラフネスにも
  使えるよう、リソースを TYPELESS で作って UNORM と UNORM_SRGB の 2 つの SRV を持つ。
...
- **マスクは高さと同じ土俵で競合する**: 重みは
  `a = 下地の高さ + (1 - マスク)`、`b = レイヤーの高さ + マスク` の比較で決まる。
  マスク 1.0 は「高さに関係なく全面を覆う」意味になるので、
  高さで勝敗を決めたいときは双方 0.5 付近にする。
  このとき上のレイヤーの基準高さが「溜まる水位」として働く。

- **法線の勾配スケール**: 生の `d(height)/d(uv)` はノイズ周波数がそのまま出て
  極端に急な法線になる。`CompositeLayer.hlsl` の `kNormalGradientScale` で丸めて、
  「法線の強さ 1.0」が妥当な見た目になるようにしている。

- **タイル評価は常用する**: プレビューでも 512 のタイルに分割して評価している。
  エクスポート時と同じ経路を通しておかないと、タイル評価が壊れたまま気づけない。
  一括評価との一致は画素単位で確認済み。

- **既定レイアウトは `ImGuiCond_FirstUseEver` で流す**: 「ini が無い初回起動だけ適用」
  にすると、後から追加したパネルが既存 ini の環境で配置されず重なる。
  毎フレーム流しても ini にあるウィンドウには効かない。
